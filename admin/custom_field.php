...
<?php
$results = array();
$query = "SELECT * FROM `".$config['db']['pre']."custom_fields` order by custom_order";
    ...
    foreach($results as $result){
        ...
        if($options != ""){
            ...
            foreach($options as $opt_id){
                $query = "SELECT * FROM `".$config['db']['pre']."custom_options` where option_id = '$opt_id' LIMIT 1";
                $query_result = mysqli_query ($mysqli,$query) OR error(mysqli_error($mysqli));
                while ($row = mysqli_fetch_array($query_result)) {
                    $option_id = $row['option_id'];
                    $option_title = stripslashes($row['title']);
