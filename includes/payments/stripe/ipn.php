...
{
    $response = array('success'=>false,'message'=>$lang['INVALID-PAYMENT_PROCESS']);
...
function quickad_stripe_payment($config,$mysqli,$lang) {
    ...
    try {
        ...
        if ( $charge->paid ) {
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                $response = array('success'=>false,'message'=>$lang['INVALID-TRANSACTION']);
                echo json_encode($response);
                exit();
            }
