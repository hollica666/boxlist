...
if(checkloggedin($config)) {
    ...
    $items = get_items($config,$_SESSION['user']['id'],"hide",false,$_GET['page'],$limit);
    $total_item = get_items_count($config,$_SESSION['user']['id'],"hide");
    $pagging = pagenav($total_item,$_GET['page'],$limit,$config['site_url'].'hidden-ads');
    // Output to template
    $page = new HtmlTemplate ('templates/' . $config['tpl_name'] . '/ad-hidden.tpl');
    $page->SetParameter ('OVERALL_HEADER', create_header($lang['HIDDEN-ADS']));
