    ...
    if( document.getElementById(element) != null ){
        ...
        if( optimizedDatabaseLoading == 1 ){
            google.maps.event.addListener(map, 'idle', function(){
                if( searchClicked != 1 ){
                    ...
                    loadData(site_url + "user-ajax.php?action=getlocHomemap");
                }
            });
        }
        else {
        ...
        function placeMarkers(markers){
            ...
            function opengmapAdBox(id, _this, i){
                $.ajax({
                    url: site_url + "user-ajax.php?action=openlocatoionPopup",
                    dataType: "html",
                    data: { id: id },
                    method: "POST",
            ...
            function openSidebarDetail(id){
                $.ajax({
                    url: site_url + "user-ajax.php?action=sidebar_detail",
                    data: { id: id },
                    method: "POST",
                    success: function(results){
            ...
            function renderResults(){
                ...
                /*if( $(".hero-section").hasClass("sidebar-grid") ){
                 var sidebarUrl = site_url + "user-ajax.php?action=sidebar_results_grid";
                 }
                 else {
                 sidebarUrl = site_url + "user-ajax.php?action=sidebar_results";
                 }
        ...
        $("[data-ajax-response='map']").on("click", function(e){
            e.preventDefault();
            var dataFile = site_url + "user-ajax.php?action=getlocHomemap";
            searchClicked = 1;
